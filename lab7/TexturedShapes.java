...
public class TexturedShapes {
    ...
    public static void Piramid(GL2 gl) {
        piramid(gl,0.5,true);
    }
    ...
    public static void triangle(GL2 gl, double side, boolean makeTexCoords) {
        double radius = side/2;
        gl.glBegin(GL2.GL_TRIANGLES);
        gl.glNormal3f(0,0,1);
        if (makeTexCoords)
            gl.glTexCoord2d(0,0);
        gl.glVertex2d(-radius * 1.5,-radius);
        if (makeTexCoords)
            gl.glTexCoord2d(1,0);
        gl.glVertex2d(radius *1.5,-radius);
        if (makeTexCoords)
            gl.glTexCoord2d(0.5, 0.5);
        gl.glVertex2d(0,radius * 2);
        gl.glEnd();
    } // end square
    
    public static void piramid(GL2 gl, double side, boolean makeTexCoords) {
    	gl.glPushMatrix();
        gl.glRotatef(-90,-1,0,0);  // This puts the textures in the orientation I want.
        gl.glPushMatrix();
        gl.glTranslated(0,0,side/2.35);
        square(gl,side * 1.5,makeTexCoords);  // Each side of the cube is a transformed square.
        gl.glPopMatrix();
        ...
        triangle(gl,side,makeTexCoords);
        gl.glPopMatrix();
        gl.glPushMatrix();
        gl.glRotatef(180,0,1,0);
        gl.glTranslated(0,0,side/2);
